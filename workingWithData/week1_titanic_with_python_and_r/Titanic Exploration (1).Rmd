...
The **factor** datatype makes much more sense for this feature. In the year 1912, it was widely held throughout Europe and the U.S. that every human could be categorized into one of two genders. `Sex`, in this dataset is used as a category, which is exactly what the **factor** datatype is meant for. The `summary` function tells us what proportion of our dataset identified as male/female.
...
Next up we have the `Pclass` feature.
